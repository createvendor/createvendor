        ...
        <>
            ...
            <div className="fixed inset-y-0 right-0 w-full md:w-[450px] bg-white z-[110] flex flex-col shadow-2xl transform transition-transform duration-500 ease-out translate-x-0 font-sans">
                <div className="p-6 border-b flex items-center justify-between bg-white sticky top-0 z-10">
                    <div>
                        <h2 className="text-lg font-bold uppercase tracking-tight text-gray-900">Your Bag</h2>
                        <p className="text-[10px] font-medium text-gray-400 uppercase tracking-widest">{items.length} Modules Registered</p>
                    </div>
                    <button
                        onClick={() => setIsCartOpen(false)}
                        className="h-8 w-8 border border-gray-100 rounded-lg flex items-center justify-center hover:bg-black hover:text-white transition-all shadow-sm"
                    >
                        <X className="h-4 w-4" />
                    </button>
                </div>
                ...
                <div className="flex-1 overflow-y-auto p-6 space-y-6 scrollbar-hide">
                    {items.length === 0 ? (
                        <div className="flex flex-col items-center justify-center h-full text-center space-y-4">
                            ...
                            <button
                                onClick={() => setIsCartOpen(false)}
                                className="px-6 py-3 bg-black text-white text-[10px] font-bold uppercase tracking-widest rounded-lg hover:bg-gray-800 transition-all active:scale-95 font-mono"
                            >
                                Re-enter Store
                            </button>
                        </div>
                        ...
                        <div className="space-y-6">
                                {items.map((item, idx) => {
                                    ...
                                    return (
                                        <div key={uniqueKey} className="flex gap-4 group border-b border-gray-50 pb-6 last:border-0 last:pb-0 font-sans">
                                            <div className="relative w-20 h-20 bg-gray-50 rounded-xl overflow-hidden flex-shrink-0 border border-gray-100 transition-transform group-hover:scale-105 shadow-sm">
                                            ...
                                            <div className="flex-1 flex flex-col pt-0.5">
                                                <div className="flex justify-between items-start">
                                                    <div className="space-y-1.5 flex-1">
                                                        <h4 className="font-bold text-gray-900 text-[13px] uppercase tracking-tight line-clamp-2 leading-tight">
                                                            {item.product.name}
                                                        </h4>
                                                        ...
                                                        {item.selectedVariants && (
                                                            <div className="flex flex-wrap gap-1.5 pt-0.5">
                                                                {Object.entries(item.selectedVariants).map(([name, value]) => (
                                                                    <span key={name} className="px-1.5 py-0.5 bg-gray-50 text-[8px] text-gray-400 font-black uppercase tracking-[0.1em] rounded-sm border border-gray-100/50">
                                                        ...
                                                        {item.extraInputs && Object.keys(item.extraInputs).length > 0 && (
                                                            <div className="pt-1.5 space-y-1">
                                                                {Object.entries(item.extraInputs).map(([label, val]) => val && (
                                                                    <div key={label} className="text-[9px] font-bold text-gray-400 uppercase tracking-widest break-all">
                                                                        {label}: <span className="text-blue-600 lowercase font-medium">{val}</span>
                                                                    </div>
                                                                ))}
                                                            </div>
                                                        )}
                                                    ...
                                                    <button
                                                        ...
                                                        className="text-gray-300 hover:text-red-500 transition-all p-1.5 -mr-1.5"
                                                    >
                                                ...
                                                <div className="mt-4 flex items-center justify-between">
                                                    <div className="flex items-center bg-gray-50 border border-gray-100 rounded-lg overflow-hidden p-0.5">
                                                        ...
                                                        <span className="w-6 text-center font-bold text-[10px] text-gray-900">{item.quantity}</span>
                                                        <button
                                                            onClick={() => updateQuantity(item.product.id, item.quantity + 1, item.selectedVariants)}
                                                    ...
                                                    <span className="font-bold text-gray-950 text-[15px] tracking-tighter">
                    ...
                    <div className="p-6 bg-white border-t space-y-6">
                        <div className="space-y-3">
                            ...
                            <div className="flex justify-between items-end">
                                <span className="text-sm font-bold uppercase tracking-tight text-gray-900">Gross Subtotal</span>
                        ...
                        <Link
                            ...
                            onClick={() => setIsCartOpen(false)}
                            className="w-full h-14 bg-black text-white text-[10px] font-bold uppercase tracking-[0.2em] rounded-xl hover:bg-gray-800 transition-all shadow-xl shadow-black/5 flex items-center justify-center group font-mono"
                        >
                            Commit To Purchase
                        </Link>
                    </div>
